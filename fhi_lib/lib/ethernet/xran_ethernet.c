...
void
xran_init_mbuf_pool(struct xran_io_cfg *io_cfg, uint32_t mtu)
{
    ...
    if (rte_eal_process_type() == RTE_PROC_PRIMARY) {
        _eth_mbuf_pool = xran_pktmbuf_pool_create("mempool", io_cfg->num_mbuf_alloc,
                MBUF_CACHE, 0, data_room_size, rte_socket_id());
        _eth_mbuf_pool_indirect = xran_pktmbuf_pool_create("mempool_indirect", io_cfg->num_mbuf_vf_alloc,
                MBUF_CACHE, 0, 0, rte_socket_id());
        _eth_mbuf_pkt_gen = xran_pktmbuf_pool_create("mempool_pkt_gen",
                io_cfg->num_mbuf_alloc, MBUF_CACHE, 0, MBUF_POOL_PKT_GEN_ELM, rte_socket_id());
    } else {
...
void xran_init_port(int p_id, struct xran_io_cfg *io_cfg, uint32_t mtu)
{
    ...
    printf("[%d] %s\n", p_id, rx_pool_name);
    _eth_mbuf_pool_vf_small[p_id] = xran_pktmbuf_pool_create(rx_pool_name, io_cfg->num_mbuf_vf_alloc,
    ...
    for (qi = 0; qi < num_rxq; qi++) {
        ret = rx_queue_setup(p_id, qi, (qi == 0) ? BURST_SIZE : nb_rxd,
